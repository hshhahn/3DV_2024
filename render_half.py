    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        if idx%2==0:
            ...
            if idx+1==len(views):
                print("End of Views!")
                break
            time_input = views[idx-1].fid.unsqueeze(0).expand(xyz.shape[0], -1)
            d_xyz, d_rotation, d_scaling = deform.step(xyz.detach(), time_input)

            time_input = views[idx+1].fid.unsqueeze(0).expand(xyz.shape[0], -1)
            d_xyz2, d_rotation2, d_scaling2 = deform.step(xyz.detach(), time_input)

            d_xyz = (d_xyz + d_xyz2) / 2.0
            d_rotation = (d_rotation + d_rotation2) / 2.0
            d_scaling = (d_scaling + d_scaling2) / 2.0
