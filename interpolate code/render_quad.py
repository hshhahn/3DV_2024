...
def render_set(model_path, load2gpu_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background, deform):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        r = idx%trainscale
        if idx -r + trainscale >= len(views):
            break
        
        fid_n = views[idx-r].fid
        time_input = fid_n.unsqueeze(0).expand(xyz.shape[0], -1)
        d_xyz_n, d_rotation_n, d_scaling_n = deform.step(xyz.detach(), time_input)

        fid_p = views[idx+trainscale-r].fid
...
def interpolate_all(model_path, load2gpt_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background, deform):
    render_path = os.path.join(model_path, name, "interpolate_all_{}_quad".format(iteration), "renders")
    depth_path = os.path.join(model_path, name, "interpolate_all_{}_quad".format(iteration), "depth")

    makedirs(render_path, exist_ok=True)
    makedirs(depth_path, exist_ok=True)

    trainscale = 5 # 학습할 때 몇 배 줄여서 학습했는지
    scale = 16 # render할 때 이미지를 몇 배 늘려서 render할지

    frame = ((200-1)//trainscale)*scale + 1
    render_poses = torch.stack([pose_spherical(angle, -30.0, 4.0) for angle in np.linspace(-180, -180, frame + 1)[:-1]],
                               0)
    to8b = lambda x: (255 * np.clip(x, 0, 1)).astype(np.uint8)
    ...
    renderings = []
    for i, pose in enumerate(tqdm(render_poses, desc="Rendering progress")):
        # fid = torch.Tensor([i / (frame - 1) * 196/199]).cuda()
        ...
        r = i%scale
        fid_n = torch.Tensor([(i-r) / (frame - 1)  * (199//trainscale * trainscale)/199]).cuda()
        time_input = fid_n.unsqueeze(0).expand(xyz.shape[0], -1)
        d_xyz_n, d_rotation_n, d_scaling_n = deform.step(xyz.detach(), time_input)

        fid_p = torch.Tensor([(i+scale-r) / (frame - 1)  * (199//trainscale * trainscale)/199]).cuda()
        time_input = fid_p.unsqueeze(0).expand(xyz.shape[0], -1)
        d_xyz_p, d_rotation_p, d_scaling_p = deform.step(xyz.detach(), time_input)

        d_xyz = (d_xyz_n * (scale-r) + d_xyz_p * r) / scale
        d_rotation = (d_rotation_n * (scale-r) + d_rotation_p * r) / scale
        d_scaling = (d_scaling_n * (scale-r) + d_scaling_p * r) / scale
    ...
    renderings = np.stack(renderings, 0).transpose(0, 2, 3, 1)
    imageio.mimwrite(os.path.join(render_path, 'video.mp4'), renderings, fps=30*scale//trainscale, quality=8)
